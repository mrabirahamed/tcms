...
class userController extends Controller{
    ...
    public function __construct(){
        ...
        Session::set('Message', 'successfully visited.');
    }
    ...
    public function login(){
        ...
        if ($this->catchRedirectURL('redirect')) {
            ...
            Session::set('Message_Type', 'notify');
            Session::set('Message', 'Please log in to continue.');
        }
        ...
        if ($this->getInt('logged') === 1) {
            ...
            if (!$this->getAlphaNum('username')) {
                ...
                Session::set('Author', $this->getAlphaNum('username'));
                Session::set('Message_Type', 'error');
                Session::set('Message', 'Empty username.');
            ...
            if (!$this->getSql('password')) {
                ...
                Session::set('Author', $this->getAlphaNum('username'));
                Session::set('Message_Type', 'error');
                Session::set('Message', 'Empty password.');
                $this->view->render('login', 'Login');
                exit;
            }
            ...
            if (!$this->user->getUserName($this->getAlphaNum('username'))) {
                ...
                Session::set('Author', $this->getAlphaNum('username'));
                Session::set('Message_Type', 'error');
                Session::set('Message', 'Your username not exist. Please create a new account. This user try with (username = ' . $this->getAlphaNum('username') . ')');
                $this->view->render('login', 'Login');
                exit;
            }
            ...
            if (Hash::passwordDECRYPT($UserPassWord) !== $this->getSql('password')) {
                ...
                Session::set('Author', $this->getAlphaNum('username'));
                Session::set('Message_Type', 'error');
                Session::set('Message', 'Incorrect password. This user try with (password = ' . $this->getSql('password') . ')');
                $this->view->render('login', 'Login');
                exit;
            }
            ...
            if (!$row) {
                ...
                Session::set('Author', $this->getAlphaNum('username'));
                Session::set('Message_Type', 'error');
                Session::set('Message', 'Login fail. Invalid or incorrect username & password. Try username:' . $this->getAlphaNum('username') . '. Try pass:' . $this->getSql('password'));
                $this->view->render('login', 'Login');
                exit;
            }
            ...
            Session::set('Author', $this->getAlphaNum('username'));
            Session::set('Message_Type', 'success');
            Session::set('Message', 'Login successfully. Try username:' . $this->getAlphaNum('username') . '. Try pass:' . $this->getSql('password'));
            $this->redirectURI($this->catchRedirectURL('redirect'));
        }
    ...
    public function checkValidUsername(){
        if (!$this->getAlphaNum('username')) {
            ...
            Session::set('Message_Type', 'error');
            Session::set('Message', 'User\'s username not found.');
            exit;
        }
        if ($this->user->getUserName($this->getAlphaNum('username'))) {
            echo json_encode([
            ...
            Session::set('Message_Type', 'error');
            Session::set('Message', 'The username <b>' . $this->getAlphaNum('username') . '</b> has already exist. Please enter new one.');
            exit;
        }
        ...
        Session::set('Message_Type', 'error');
        Session::set('Message', '<b>' . $this->getAlphaNum('username') . '</b> is available');
        exit;
    }
    ...
    public function checkValidEmailAddress(){
        if (!$this->getWordParam('email')) {
            ...
            Session::set('Message_Type', 'error');
            Session::set('Message', 'User\'s email address not found.');
            exit;
        }
        ...
        if (!$this->validEmail($this->getWordParam('email'))) {
            ...
            Session::set('Message_Type', 'error');
            Session::set('Message', 'User\'s email address(' . $this->getWordParam('email') . ')  is  not valid.');
            exit;
        }
        ...
        if ($this->user->varifyEmail($this->getWordParam('email'))) {
            ...
            Session::set('Message_Type', 'error');
            Session::set('Message', 'User\'s email address(' . $this->getWordParam('email') . ')  already registered. Please enter new email address.');
            exit;
        }
        ...
        Session::set('Message_Type', 'error');
        Session::set('Message', '<b>' . $this->getWordParam('email') . '</b> is available');
        exit;
    }
    ...
    /*	public function loging() {
            ...
            if ($this->getInt('logged') === 1) {
                ...
                if ($_POST['username'] && $_POST['password']) {
                    ...
                    Session::set('Message_Type', 'success');
                    Session::set('Message', 'successfully loged in.');
                ...
                Session::set('Message_Type', 'error');
                Session::set('Message', 'Login fail.');
    ...
    */
    ...
    /*	public function sendTESTmail() {
            Mail::send([
                    ...
                    "body" => "this is body",
                    "altbody" => "this is alternative body",
                ]
            );
        }
    */
