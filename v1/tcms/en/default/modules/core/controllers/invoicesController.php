...
class invoicesController extends Controller
{
    ...
    {
        //$this->invoices->dropTable();
        //$this->invoices->initilaizeTableForFirstUseOnly();
    ...
    public function addInvoice()
    {
        ...
        if (count($data) > 0) {
            ...
            if ($data->btnName === 'Save') {
                $this->invoices->insertInvoice($this->filterInt($data->client), $this->filterInt($data->sls_mn), Session::get('id_user'), Session::get('username'), Session::get('f_name') . ' ' . Session::get('l_name'));
            ...
            /*if ($data->btnName === 'Update') {
                ...
                echo json_encode([['type' => 'success', 'message' => ucfirst($this->productdetails->getBrandName($prd_dtl[0]['brand'])) . '&nbsp;' . ucfirst($this->productdetails->getItemName($prd_dtl[0]['item'])) .  ' (Model: '. $prd_dtl[0]['Model'] . ')&nbsp;(Serial: '. $prd_dtl[0]['serial'] . ')&nbsp; price: ' . $prd_dtl[0]['price'] .' BDT to ' . ucfirst($this->productdetails->getBrandName($data->brand)) . '&nbsp;' . ucfirst($this->productdetails->getItemName($data->item)) .  ' (Model: '. $data->modelNumber . ')&nbsp;(Serial: '. $data->serialNumber . ')&nbsp; price: ' . $data->unitPrice .' BDT updated successfully...'],]);
                Tracker::addEvent(array(
    ...
    public function manageBill(){
        ...
        if (count($data) > 0) {
            ...
            if ($data->mode === 'create' OR $data->mode === 'update') {
                $this->invoices->manageBill($this->getTextnonPOST($data->mode),$this->filterInt($data->invoice), $this->filterInt($data->client), $this->filterInt($data->bill), Session::get('id_user'), Session::get('username'), Session::get('f_name') . ' ' . Session::get('l_name'));
    ...
    public function doprint($invoice){
        ...
        $this->view->render('doprint', 'Invoice_print_page');
    }
    //--------------------------------------------------
}
