            ...
            {foreach item = data  from = $product_details}
                <tr>
                    <td class="text-align-center">
                        <label><input type="checkbox" id="productdetails-select" class="check_box" data-id="{$data.id}" data-prd-detls-item="{$data.itemId}" data-prd-detls-brand="{$data.brandId}" data-modelNumber="{$data.model}" data-serialNumber="{$data.serial}" data-unitPrice="{$data.price}" data-warrantyTime="{$data.warrenty}" data-ability="{$data.ability}"/></label></td>
                    </td>
                    ...
                    <td class="text-align-center">{$data.warrenty}</td>
                    <td class="text-align-center">
                        {if ($data.ability === 'available')}<a href="javascript:void(0)" id="changeProductDetailAbility" data-id="{$data.id}" data-curent_status="{$data.ability}" class="button button-xs button-success" title="Click to make unavailable"><i class="far fa-check-circle"></i></a>{else}<a href="javascript:void(0)" id="changeProductDetailAbility" data-id="{$data.id}" data-curent_status="{$data.ability}" class="button button-xs button-danger" title="Click to make available"><i class="far fa-times-circle"></i></a>{/if}
                    </td>
                    <td class="text-align-center">
                        <a href="javascript:void(0);" id="productdetails-edit-btn" class="button button-xs button-success" data-id="{$data.id}" data-prd-detls-item="{$data.itemId}" data-prd-detls-brand="{$data.brandId}" data-modelNumber="{$data.model}" data-serialNumber="{$data.serial}" data-unitPrice="{$data.price}" data-warrantyTime="{$data.warrenty}" data-ability="{$data.ability}"> <i class="far fa-edit"></i></a>&nbsp;&nbsp;
                        <a href="javascript:void(0);" id="productdetails-delete-btn" class="button button-xs button-danger" data-id="{$data.id}"><i class="far fa-trash-alt"></i></i></a>
                    </td>
                </tr>
            {/foreach}
        {/if}
