...
class ProductDetailsManager extends Mishusoft {
    constructor() {
        super();
        this.AllBranchesURL = this.AppHostAddress + 'office/productdetails/getBranches';
        this.AllAvailableItemsURL = this.AppHostAddress + 'office/items/getAvailableItemsOrderByName';
        this.AllBrandsURL = this.AppHostAddress + 'office/brands/getBrandsOrderByName';
        this.viewBranchesForProductDetails();
        this.viewItemsForProductDetails();
        this.viewBrandsForProductDetails();
    }

    viewBranchesForProductDetails() {
        this.ajax = new XMLHttpRequest();
        this.ajax.open(this.method, this.AllBranchesURL, this.asynchronous);
        this.ajax.send();
        ...
        this.ajax.onreadystatechange = function () {
            if (this.readyState === 4 && this.status === 200) {
                ...
                this.htmlpad = Mishusoft.detectElement('prd-detls-branch');
                //html value for <body>
                this.html = '';
    ...
    saveProductDetails() {
        ...
        if (command === 'Save') {
            ...
            data = {
                security_code: 1,
                branch: Mishusoft.detectElement('prd-detls-branch').value,
...
$('#productdetails-data-table').on('click', '.page', function () {
    let url = 'office/productdetails/productDetailsPaginationAJAX';
...
$(document).on('click', '#productdetails-add-btn', function () {
    ...
    Mishusoft.detectElement('prd-detls-branch').value = '';
    Mishusoft.detectElement('prd-detls-item').value = '';
    Mishusoft.detectElement('prd-detls-brand').value = '';
...
$(document).on('click', '#productdetails-select', function () {
    if (this.checked) {
        ...
        Mishusoft.detectElement('prd-detls-branch').value = $(this).attr('data-prd-detls-branch');
...
$(document).on('click', '#changeProductDetailAbility', function () {
    ...
    $(document).on('click', '#message-done-btn', function () {
        if (commandBtn === 'Change') {
            ...
            let ajax = new XMLHttpRequest();
            ajax.open("POST", _root_ + 'office/productdetails/ChangeProductAbility', true);
            ajax.setRequestHeader("Content-type", "application/json;charset=UTF-8");
            ajax.send(JSON.stringify(data)); // Make sure to stringify
            //receiving response from ajax
            ajax.onreadystatechange = function () {
...
$(document).on('click', '#productdetails-delete-btn', function () {
    ...
    $(document).on('click', '#message-done-btn', function () {
        if (commandBtn === 'Delete') {
            ...
            let ajax = new XMLHttpRequest();
            ajax.open("POST", _root_ + 'office/productdetails/deleteProductDetail', true);
            ajax.setRequestHeader("Content-type", "application/json;charset=UTF-8");
            ajax.send(JSON.stringify(data)); // Make sure to stringify
            //receiving response from ajax
            ajax.onreadystatechange = function () {
